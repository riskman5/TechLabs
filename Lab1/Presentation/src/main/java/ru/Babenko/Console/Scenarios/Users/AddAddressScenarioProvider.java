...
@Component
@Scope("prototype")
public class AddAddressScenarioProvider implements ScenarioProvider {
    private final UserService userService;
    private final CurrentUserService currentUserService;
    private final CurrentBankAccountService currentBankAccountService;

    public AddAddressScenarioProvider(UserService userService, CurrentUserService currentUserService, CurrentBankAccountService currentBankAccountService) {
    ...
    public Optional<Scenario> tryGetScenario() {
        ...
        if (currentUserService.getUser().get().getAddress().isPresent()) {
            return Optional.empty();
        }

        return Optional.of(new AddAddressScenario(userService));
    }
}
