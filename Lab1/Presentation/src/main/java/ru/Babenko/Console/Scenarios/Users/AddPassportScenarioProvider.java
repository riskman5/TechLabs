...
@Component
@Scope("prototype")
public class AddPassportScenarioProvider implements ScenarioProvider {
    private final UserService userService;
    private final CurrentUserService currentUserService;
    private final CurrentBankAccountService currentBankAccountService;

    public AddPassportScenarioProvider(UserService userService, CurrentUserService currentUserService, CurrentBankAccountService currentBankAccountService) {
    ...
    public Optional<Scenario> tryGetScenario() {
        ...
        if (currentUserService.getUser().get().getPassportNumber().isPresent()) {
            return Optional.empty();
        }

        return Optional.of(new AddPassportScenario(userService));
    }
}
